...
fn main() -> Fallible<()> {
    ...
    process_command(command, &vault, &vault)
}
...
fn selected_profile(
    ...
    let profile_opt = my_profile_option
        .or(vault.get_active()?)
        .and_then(|profile_id| store.get(&profile_id));
...
{
    let profile_ptr = selected_profile(vault, store, my_profile)?;
...
fn process_command(command: Command, vault: &ProfileVault, store: &ProfileStore) -> Fallible<()> {
    match command {
        Command::Create(CreateCommand::Link {
            my_profile_id,
            peer_profile_id,
        }) => {
            on_profile(vault, store, my_profile_id, |profile| {
        ...
        Command::Create(CreateCommand::Profile) => {
            ...
            let created_profile_ptr = store.create(&new_profile_id)?;
