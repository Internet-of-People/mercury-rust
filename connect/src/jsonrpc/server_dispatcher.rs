...
use jsonrpc_core::{serde_json as json, types, MetaIoHandler, Metadata, Params};
use jsonrpc_pubsub::{
    PubSubHandler, PubSubMetadata, Session as PubSubSession, Subscriber, SubscriptionId,
};
use log::*;
...
impl SessionData {
    pub fn new(transport_tx: mpsc::Sender<String>) -> Self {
        ...
        Self {
            pubsub_session: Arc::new(PubSubSession::new(transport_tx)),
            dapp_session: None,
            cancel_events: None,
        }
    }
...
impl JsonRpcServer {
    ...
    where
        ...
        C: 'static
            + Decoder<Item = String, Error = ::std::io::Error>
            + Encoder<Item = String, Error = ::std::io::Error>,
    {
        ...
        let fwd_req_fut = net_stream
            ...
            .forward(
                req_sink
                    .sink_map_err(|e| warn!("Failed to forward request or notification: {}", e)),
            )
            .map(|(_stream, _sink)| ());
        ...
        let fwd_resp_fut = resp_stream
            .forward(
                net_sink.sink_map_err(|e| warn!("Failed to send response or notification: {}", e)),
            )
            .map(|(_stream, _sink)| ());
    ...
    pub fn serve_client_requests<I>(
        &self,
        req_stream: I,
        resp_sink: mpsc::Sender<String>,
    ) -> AsyncResult<(), ()>
    where
        I: 'static + Stream<Item = String>,
        //O: 'static + Sink<SinkItem=String> + Clone,
    {
        let session = Session::new(resp_sink.clone());

        let dispatcher = self.pubsub_dispatcher.clone();
        let client_fut = req_stream.map_err(|_e| ()).for_each(move |line| {
            let sender = resp_sink.clone();
            dispatcher.handle_request(&line, session.clone()).and_then(move |resp_opt| {
                match resp_opt {
                    None => Either::A(Ok(()).into_future()),
                    Some(response) => {
                        Either::B(sender.send(response).map(|_sink| ()).map_err(|_e| ()))
                    }
                }
            })
        });
...
pub fn create_dispatcher(
    ...
    dispatcher.add_method_with_meta("get_session", {
        ...
        move |params: Params, mut meta: Session| {
            let param_map = match params {
                ...
                Params::None => {
                    return Either::A(
                        Err(types::Error::new(types::ErrorCode::InvalidParams)).into_future(),
                    );
                }
                Params::Array(_) => {
                    return Either::A(
            ...
            let req = match serde_json::from_value::<api::GetSessionRequest>(json::Value::Object(
                param_map,
            )) {
                Ok(req) => req,
                Err(e) => {
    ...
    pubsub.add_subscription(
        ...
        ("subscribe_events", move |params: Params, mut meta: Session, subscriber: Subscriber| {
            ...
            let subscribe_fut = fwd_events_fut
                    .select(cancel_rx.map_err(|_e| ())) // TODO
                    .map(|((), _pending)| ())
