...
impl Connecting
{
    ...
    {
        let connect_fut = UnixStream::connect(address, handle).into_future()
            .map( |stream| {
                let (rx, tx) = stream.split();
                DuplexChannel::new( Box::new(rx), Box::new(tx) )
            } );

        let timeout_fut = Timeout::new(NETWORK_TIMEOUT, handle).into_future()
            .and_then(|_| Err( io::Error::from(io::ErrorKind::TimedOut) ) )
            .or_else( |err| Err( io::Error::new(io::ErrorKind::Other, err) ) );

        let fut = connect_fut.select(timeout_fut)
...
{
    fn poll_start<'s, 'c>(_start: &'s mut RentToOwn<'s, Start>, context: &'c mut RentToOwn<'c, Context>)
        -> Poll<AfterStart, ClientFsmError>
    {
        transition!( Connecting::new(&context.address, &context.handle) )
    }

    fn poll_connecting<'s, 'c>(connecting: &'s mut RentToOwn<'s, Connecting>, context: &'c mut RentToOwn<'c, Context>)
    ...
    fn poll_backoff<'s, 'c>(backoff: &'s mut RentToOwn<'s, Backoff>, context: &'c mut RentToOwn<'c, Context>)
    ...
    fn poll_connected<'s, 'c>(connected: &'s mut RentToOwn<'s, Connected>, context: &'c mut RentToOwn<'c, Context>)
