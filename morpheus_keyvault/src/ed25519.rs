...
use super::{AsymmetricCrypto, Seed, PublicKey, PrivateKey, KeyDerivationCrypto, ExtendedPrivateKey, ExtendedPublicKey};
...
impl KeyDerivationCrypto for Ed25519
{
    ...
    fn master(seed: &Seed) -> EdExtPrivateKey { unimplemented!() }
}


impl PublicKey<Ed25519> for EdPublicKey {
    fn key_id(&self) -> KeyId { unimplemented!() }
    fn verify(&self, data: &[u8], sig: Signature) -> bool { unimplemented!() }
}


impl PrivateKey<Ed25519> for EdPrivateKey {
    fn public_key(&self) -> EdPublicKey { unimplemented!() }
    fn sign(&self, data: &[u8]) -> Signature { unimplemented!() }
}


impl ExtendedPrivateKey<Ed25519> for EdExtPrivateKey {
    fn derive_normal_child(&self, idx: i32) -> Fallible<EdExtPrivateKey> { unimplemented!() }
    fn derive_hardened_child(&self, idx: i32) -> Fallible<EdExtPrivateKey> { unimplemented!() }
    fn neuter(&self) -> EdExtPublicKey { unimplemented!() }
    fn as_private_key(&self) -> EdPrivateKey { unimplemented!() }
}


impl ExtendedPublicKey<Ed25519> for EdExtPublicKey {
    fn derive_normal_child(&self, idx: i32) -> Fallible<EdExtPublicKey> { unimplemented!() }
    fn as_public_key(&self) -> EdPublicKey { unimplemented!() }
}

#[cfg(test)]
mod tests {
    use crate::{Seed, KeyDerivationCrypto, ExtendedPrivateKey};
    use super::Ed25519;
