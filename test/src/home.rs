...
use mercury_home_protocol::mercury_capnp::{
    client_proxy::HomeClientCapnProto, server_dispatcher::HomeDispatcherCapnProto,
};
...
impl TestClient {
    fn new(
        ...
        match test_mode {
            TestMode::Direct => {
                Self::direct(ownprofile, client_signer, home_server, home_signer, home_profile)
            }
            TestMode::Memsocket => Self::memsocket(
                ownprofile,
                client_signer,
                home_server,
                home_signer,
...
fn test_home_events(mut setup: TestSetup) {
    ...
    let pair_result =
        setup.reactor.run(setup.testclient.home_connection.pair_request(half_proof)).unwrap();
...
fn test_home_call(mut setup: TestSetup) {
    ...
    let caller_testclient = TestClient::new(
        ...
        caller_ownprofile,
        caller_signer.clone(),
        setup.home_server.clone(),
        setup.home_signer.clone(),
        &setup.home_profile.clone(),
        setup.reactor.handle(),
    );
    ...
    let (forward_sink_returned, backwards_sink_returned_vec) =
        setup.reactor.run(call_and_answer).unwrap();
