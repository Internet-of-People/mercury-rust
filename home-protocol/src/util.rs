...
pub fn parse_config<T: StructOpt>(config_path: &str) -> T {
    ...
    let file_args = read_config_file(config_path).unwrap_or(Vec::new()); // TODO should continue if cfg file was not specified but fail otherwise
...
fn read_config_file(config_path: &str) -> Result<Vec<String>, ()> {
    ...
    let file_keyvals: HashMap<String, String> = toml::from_str(&file_contents).map_err(|e| {
