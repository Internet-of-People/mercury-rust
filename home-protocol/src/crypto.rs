...
impl ProfileValidator for MultiHashProfileValidator {
    fn validate_profile(
        ...
        let id_hashalgo =
            multihash::decode(profile_id.0.as_slice()).context(ErrorKind::HashDecodeFailed)?.alg;
...
impl Ed25519Signer {
    pub fn new(private_key: &PrivateKey) -> Result<Self, Error> {
        let seed = ed25519::Seed::from_bytes(private_key.0.clone())
            .context(ErrorKind::SignerCreationFailed)?;
        ...
        Ok(Self { public_key: public_key.to_owned(), profile_id: profile_hash, signer })
    }
}
...
impl Signer for Ed25519Signer {
    ...
    fn sign(&self, data: &[u8]) -> Signature {
        let signature = self.signer.sign(data).unwrap(); // TODO ERROR HANDLING how to handle possibly returned errors here?
...
impl SignatureValidator for Ed25519Validator {
    fn validate_signature(
        ...
        let pubkey = signatory::ed25519::PublicKey::from_bytes(public_key.0.as_slice())
            .context(ErrorKind::SignatureValidationFailed)?;
